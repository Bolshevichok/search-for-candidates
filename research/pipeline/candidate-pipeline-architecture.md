...
| `defenses[]` (`date`, `specialty`, `branch`, `dissertation_type`, `topic`, `defend_org`, `is_pilot`) | ВАК | массив — один человек может иметь несколько записей защит; у `site_no_vak` может быть пусто. `specialty` — как в самом API: одна строка «код - название» (например `"5.10.3. - Виды искусства"`), не отдельный числовой код. `branch` — более крупная «отрасль науки» текстом (например `"Технические науки"`). Оба поля уже приходят готовыми в ответе API (см. `vak-analysis.md`, пример карточки) — просто прокидываем их дальше как есть |
